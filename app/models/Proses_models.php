        ...
        */
        ...
        try
        {
            ...
            */
            ...
            $this
                ->db
                ->bind('nama_jenis', $nama);
            $this
                ->db
                ->bind('kode_jenis', $kode_jenis);
            $this
                ->db
        ...
        */
        ...
        try
        {
            ...
            */
            ...
            $this
                ->db
                ->bind('tanggal_pinjam', $date);
            $this
                ->db
                ->bind('tanggal_kembali', $kembali);
            $this
                ->db
                ->bind('id_auth', $data['peminjam']);
            $this
                ->db
                ->bind('lama_pinjam', $data['pinjam']);
            $this
                ->db
                ->bind('status', '1');
            $this
            ...
            */
            ...
            $this
                ->db
                ->bind('id_barang', $pilih_barang);
            $this
                ->db
                ->bind('id_peminjam', $pinjam['id_peminjam']);
            $this
                ->db
                ->bind('jumlah_pinjam', $jumlah);
            $this
                ->db
                ->execute();
        ...
        */
        ...
        try
        {
            ...
            */
            ...
            $this
                ->db
                ->bind('nama_ruang', $nama);
            $this
                ->db
                ->bind('kode_ruang', $kode_ruang);
            $this
                ->db
                ->bind('keterangan', $keterangan);
            $this
        ...
        */
        ...
        try
        {
            ...
            */
            ...
            $this
                ->db
                ->bind('no_induk', $no_induk);
            $this
                ->db
                ->bind('tgl_daftar', $date);
            $this
                ->db
                ->bind('id_level', $level);
            $this
                            ...
                            if (move_uploaded_file($file['tmp_name'], $destination . $fileNewName))
                            {
                                ...
                                try
                                {
                                    ...
                                    */
                                    ...
                                    $this
                                        ->db
                                        ->bind('jumlah', $jumlah_barang);
                                    $this
                                        ->db
                                        ->bind('tanggal_masuk', $date);
                                    $this
                                        ->db
                                        ->bind('kondisi', $kondisi);
                                    $this
                                        ->db
                                        ->bind('deskripsi', $keterangan);
                                    $this
                                        ->db
                                        ->bind('id_jenis', $jenis);
                                    $this
                                        ->db
                                        ->bind('id_ruang', $ruangan);
                                    $this
                                        ->db
                                        ->bind('id_auth', $id_auth);
                                    $this
                                        ->db
    ...
    */
    ...
    public function Ulike($id)
    {
        ...
        if ($cek > 0)
        {
            ...
            $this
                ->db
                ->bind('like_count', $barang['like_count'] - '1');
            $this
                ->db
                ->execute();
    ...
    public function Uprofile($data)
    {
        if (empty($data['kelas']) || $data['kelas'] === "0" || empty($data['jurusan']) || $data['jurusan'] === '0' || empty($data['index']) || $data['index'] === '0' || empty($data['provinsi']) || $data['provinsi'] === '0' || empty($data['kabupaten']) || $data['kabupaten'] === '0' || empty($data['kecamatan']) || $data['kecamatan'] === '0')
        {
            $id_encrypsi = Encripsi::encode('encrypt', $data['id']);
            Flasher::setFlash('Data Harus Di isi ', 'Tidak Boleh Kosong', 'error');
            header('Location: ' . BASE_URL . '/profile/' . $id_encrypsi);
            exit();
        }
        else
        {
            ...
            $username = htmlspecialchars($data['username'], ENT_QUOTES);
            $nama = htmlspecialchars($data['nama'], ENT_QUOTES);
            $password_lama = htmlspecialchars($data['password_lama'], ENT_QUOTES);
            ...
            $index = htmlspecialchars($data['index'], ENT_QUOTES);
            $alamat = htmlspecialchars($data['alamat'], ENT_QUOTES);
            $provinsi = htmlspecialchars($data['provinsi'], ENT_QUOTES);
            $kabupaten = htmlspecialchars($data['kabupaten'], ENT_QUOTES);
            $kecamatan = htmlspecialchars($data['kecamatan'], ENT_QUOTES);
            $val = $this->model('Get_models')
                ->ambilDataBy('id_auth', $id, 'tb_identitas');
            if ($val > 0 && $val != 0)
            {
                if (empty($password_lama) && empty($password_baru))
                {
                    $query = "UPDATE auth SET username = :username, nama = :nama WHERE id_auth = $id";
                    try
                    {
                        ...
                        $query1 = "UPDATE tb_identitas SET  alamat = :alamat , provinsi = :provinsi , kota = :kota , kecamatan = :kecamatan ,kelas =:kelas ,jurusan = :jurusan , index_kelas = :index_kelas WHERE id_auth = $id";
                        $this
                            ->db
                            ->query($query1);
                        $this
                        ...
                        $this
                            ->db
                            ->bind('provinsi', $provinsi);
                        $this
                            ->db
                            ->bind('kota', $kabupaten);
                        $this
                            ->db
                            ->bind('kecamatan', $kecamatan);
                        $this
                            ->db
                            ->bind('kelas', $kelas);
                        $this
                ...
                {
                    $datapw = $this->model('Get_models')
                        ->ambilDataBy('id_auth', $id, 'auth');
                    if (password_verify($password_lama, $datapw['password']))
                    {
                        $password_baruv2 = password_hash($password_baru);
                        $query = "UPDATE auth SET username = :username, nama = :nama , password =:password WHERE id_auth = $id";
                        try
                        {
                    ...
                    {
                        $id_encrypsi = Encripsi::encode('encrypt', $id);
                        Flasher::setFlash('Password Lama Anda ', 'Salah !!', 'error');
                        header('Location: ' . BASE_URL . '/profile/' . $id_encrypsi);
                        exit();
                    }
            ...
            else
            {
                if (empty($password_lama) && empty($password_baru))
                {
                    ...
                    try
                    {
                        ...
                        $query1 = "INSERT INTO tb_identitas VALUES ('',:id_auth ,:alamat ,:provinsi ,:kota ,:kecamatan ,:kelas ,:jurusan ,:index_kelas, :gambar)";
                        $this
                            ->db
                            ->query($query1);
                        $this
    ...
    public function Ufoto($data)
    {
        $id = htmlspecialchars($data['id']);
        $id_auth = Encripsi::encode('encrypt', $id);
        $file_max_weight = 10000000; //limmit gambar byte
        $ok_ext = array(
            'jpg',
            'png',
            'gif',
        ...
        $destination = "C:/xampp/htdocs/Inventaris_skensa/public/img/theme/"; // simpen dmana nantik
        $file = $_FILES['gambar'];
        ...
        if ($file['error'] == 0)
        {
            if (in_array($file_extension, $ok_ext))
            {
                if ($file_weight <= $file_max_weight)
                {
                    ...
                    if (move_uploaded_file($file['tmp_name'], $destination . $fileNewName))
                    {   
                        $query = "UPDATE tb_identitas SET foto =:foto WHERE id_auth = $id";
                        try {
                            $this->db->query($query);
                            $this->db->bind('foto', $fileNewName);
                            $this->db->execute();
                            return ['status' => true];
                        } catch (Exception $e) {
                            return ['status' => false, 'msg' => $e->getMessage()];
                ...
                {
                    Flasher::setFlash('Gambar Lebih Dari 10 mb', ' !!', 'error');
                    header('Location: ' . BASE_URL . '/profile/'.$id_auth);
                    exit();
                }
            }
            else
        ...
        {
            Flasher::setFlash('Tologn Masukkan Gambar ', ' !!', 'error');
            header('Location: ' . BASE_URL . '/profile/'.$id_auth);
            exit();
        }
    }
    ...
    */
    ...
    public function kembali($id)
    {
        ...
        */
        ...
        $data = $this->model('Get_models')
            ->ambilDatapinjamOne('detail_pinjam', $id, 'id_peminjam');
        ...
        try
        {
            ...
            $this
                ->db
                ->bind('tanggal_pinjam', $data['tanggal_pinjam']);
            $this
                ->db
                ->bind('jatuh_tempo', $data['tanggal_kembali']);
            $this
                ->db
                ->bind('id_auth', $data['id_auth']);
            $this
                ->db
                ->bind('denda', $denda);
            $this
                ->db
                ->bind('id_detail_pinjam', $data['id_detail_pinjam']);
            $this
                ->db
                ->execute();
    ...
    public function proses_pinjam($data)
    {
        ...
        try
        {
            ...
            */
            ...
            $this
                ->db
                ->bind('tanggal_kembali', '0000-00-00');
            $this
                ->db
                ->bind('id_auth', $data['id_auth']);
            $this
            ...
            */
            ...
            $this
                ->db
                ->bind('id_barang', $data['id_barang']);
            $this
                ->db
                ->bind('id_peminjam', $pinjam['id_peminjam']);
            $this
                ->db
                ->bind('jumlah_pinjam', $data['jumlah_barang']);
            $this
                ->db
                ->execute();
