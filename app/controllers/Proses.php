...
class Proses extends Controller
{
    ...
    */
    ...
    public function Uprofile()
    {
        ...
        if ($add['status'])
        {
            $id_encrypsi = Encripsi::encode('encrypt', $_POST['id']);
            Flasher::setFlash("User Berhasil ", "Di Update", "success");
    ...
    public function Ufoto()
    {
        $add = $this->model('Proses_models')
            ->Ufoto($_POST);
        if ($add['status'])
        {
            $id_encrypsi = Encripsi::encode('encrypt', $_SESSION['auth']);
            Flasher::setFlash("User Berhasil ", "Di Update", "success");
            header('Location: ' . BASE_URL . '/profile/' . $id_encrypsi);
            exit();
        }
    ...
    */
    ...
    {
        echo json_encode($this->model('Get_models')
            ->ambilDataBy('id_barang', $_POST['id'], 'tb_barang'));
    }

    public function getdetail()
    {
        echo json_encode($this->model('Get_models')
            ->ambilDataInBy('id_barang', $_POST['id'], 'tb_barang'));
    }

    public function getubahjenis()
    {
        echo json_encode($this->model('Get_models')
            ->ambilDataBy('id_jenis', $_POST['id'], 'tb_jenis'));
    }

    public function getubahruang()
    {
        echo json_encode($this->model('Get_models')
            ->ambilDataBy('id_ruang', $_POST['id'], 'tb_ruang'));
    }

    public function getubahuser()
    {
        echo json_encode($this->model('Get_models')
            ->ambilDataBy('id_auth', $_POST['id'], 'auth'));
    }

    public function getpinjam()
    {
        echo json_encode($this->model('Get_models')
            ->ambilDatapinjamOne('detail_pinjam', $_POST['id'], 'id_peminjam'));
    }

    public function getkembali()
    {
        echo json_encode($this->model('Get_models')
            ->ambilKembaliBy('tb_kembali', $_POST['id']));
    }

    public function notif()
    {
        echo json_encode($this->model('Get_models')
            ->countPinjam('tb_pinjam'));
    }
